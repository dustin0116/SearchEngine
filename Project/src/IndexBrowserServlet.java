...
public class IndexBrowserServlet extends HttpServlet {
	...
	/** Title of web page. */
	private static final String TITLE = "Search Engine - Show Index";
	...
	/**
	 * Constructor for the index browser servlet.
	 *
	 * @param index the inverted index
	 */
	public IndexBrowserServlet(ThreadSafeInvertedIndex index) {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		out.printf("	<section class=\"section\">%n");
		out.printf("		<div class=\"container\">%n");
		out.printf("			<h2 class=\"title\">Display Inverted Index</h2>%n");
		...
		if (index.getWords().size() == 0) {
			...
			for (var word : index.getWords()) {

				out.printf("		<details>");
				out.printf("		<summary>%s</summary>%n", word);
				out.printf("			<div class=\"content\">%n");
				out.printf(
						"				<ul style=\"text-align:left; list-style-position:outside; list-style-type: lower-roman\">");

				for (var location : index.getLocations(word)) {
					out.printf("				<li><a href=\"%s\" target=\"_blank\">%s</a></li>%n", location,
							location);
				}
